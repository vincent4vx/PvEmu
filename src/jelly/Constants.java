package jelly;

public class Constants {

    public final static String NAME = "PvEmu";
    public final static String VERSION = "0.9.1a";
    public final static int REV = 66;
    public final static String DOFUS_VER = "1.19.0";
    public final static int DOFUS_VER_ID = 1190;
    public final static int SERV_ID = 1;
    public final static String CONFIG_FILE = "jelly.conf";
    public final static String COLOR_RED = "c10000";
    public final static int MAX_IDLE_TIME = 900; //15 minutes
}
